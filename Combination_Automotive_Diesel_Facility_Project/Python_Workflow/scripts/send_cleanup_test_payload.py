...
def make_sample_deleted_releases(n_deleted: int, max_assets: int) -> list[str]:
    ...
    for i in range(1, n_deleted + 1):
        ...
        assets = [f'<{html}/assets/{a}|asset-{a}.png>' for a in range(1, min(max_assets, 3) + 1)]
        if max_assets > 3:
...
def build_payload(keep_per_pr: int, min_age_days: int, deleted_releases: list[str], repo: str, run_id: str) -> dict:
    max_list = 10
    listItems = deleted_releases or []
    payload_blocks = []
    payload_blocks.append({"type": "header", "text": {"type": "plain_text", "text": "Draft Release Cleanup Summary"}})
    payload_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"*Retention:* keep {keep_per_pr} per PR · delete drafts older than {min_age_days} days"}})
    payload_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": "This cleanup keeps the repository tidy and prevents clutter from automated PR runs. See <https://github.com/73junito/Combination-Automotive-Diesel-Mechanics-Facility/blob/chore/types-mypy-py311/Combination_Automotive_Diesel_Facility_Project/Python_Workflow/README.md|Cleanup docs> for details."}})

    # Summary
    deleted_count = len(listItems)
    payload_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"*Summary:* {deleted_count} draft release{'' if deleted_count==1 else 's'} deleted"}})

    # Per-release sections with dividers
    for idx, item in enumerate(listItems):
        payload_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": item}})
        if idx != len(listItems) - 1:
            payload_blocks.append({"type": "divider"})

    payload_blocks.append({"type": "context", "elements": [{"type": "mrkdwn", "text": f"Repository: {repo} • Workflow: cleanup-draft-releases • Run: {run_id}"}]})
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument('--deleted', type=int, default=2, help='Number of deleted releases to synthesize')
    p.add_argument('--max-assets', type=int, default=3, help='Max assets per release (for sample)')
    p.add_argument('--output', default='docs/cleanup_test_payload.json', help='Write payload JSON to this path')
    ...
    payload = build_payload(keep_per_pr=3, min_age_days=14, deleted_releases=deleted_items, repo=repo, run_id=run_id)
